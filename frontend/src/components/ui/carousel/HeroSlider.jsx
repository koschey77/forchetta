...
export default function HeroSlider() {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-choco-dark">
        ...
        <div className="flex w-full">
          {slides.map((slide) => (
            <div key={slide.id} className="relative flex-[0_0_100%] min-w-0 w-full">
              ...
              <div className="absolute inset-0 flex flex-col justify-end items-center pb-[70px] sm:pb-[65px] md:pb-[65px] lg:pb-[65px]">
                <Link 
                  to={slide.link}
      ...
      {/* Pagination Dots */}
      <div className="absolute bottom-[40px] sm:bottom-[30px] md:bottom-8 left-0 right-0 flex justify-center items-start gap-[20px]">
        {slides.map((_, index) => (
          <button
            key={index}
