...
const ViewedProducts = () => {
  const { viewedIds, clearViewed } = useViewedStore();
  const { addToCart } = useCartStore();
  const navigate = useNavigate();
  
  const { data, isLoading } = useQuery({
    queryKey: ['viewedProducts', viewedIds],
    queryFn: () => productsAPI.getMany({ ids: viewedIds, limit: 24 }),
  ...
  // Сортируем товары так, чтобы они отображались в том порядке, в котором мы их просматривали (от новых к старым)
  const products = [...productsResponse].sort((a, b) => {
    return viewedIds.indexOf(a._id) - viewedIds.indexOf(b._id);
  });

  if (isLoading && viewedIds.length > 0) {
    return (
      ...
      <div className="flex flex-col justify-center items-center px-[15px] sm:px-5 pt-[30px] sm:pt-5 pb-16 gap-[35px] w-full min-h-[321px] sm:min-h-[580px] rounded-[10px]">
        <div className="flex flex-col items-center gap-[35px] w-full max-w-[447px]">
          ...
          <div className="flex flex-col items-center gap-0 sm:gap-[2px] w-[205px] sm:w-full">
            <img 
              src="/viewedProducts.png" 
              alt="Порожній список переглянутих товарів" 
            ...
            <h2 className="font-montserrat font-light sm:font-semibold text-[18px] sm:text-[24px] leading-[22px] sm:leading-[29px] text-choco-light text-center w-[205px] sm:w-auto">
              Наразі ви не переглянули жодного товару!
            </h2>
          </div>
  ...
  const handleClearAll = () => {
    clearViewed();
    toast.success('Список переглянутих товарів очищено');
  };
